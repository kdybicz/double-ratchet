...
describe("double ratchet", () => {
  ...
  describe("Alice and Bob", () => {
    ...
    test("Send a message back and forth", () => {
      ...
      // and
      const alice = DoubleRatchet.fromPublicKey(rootKey, keyPair.publicKey.toString("hex"));
      ...
      // then
      expect(alice.RatchetDecrypt(headerB1, messageB1, ad)).toEqual("Hi Alice!");
    });
    ...
    test("Skip a single message", () => {
      ...
      // then
      expect(alice.RatchetDecrypt(headerB2, messageB2, ad)).toEqual("Hi Alice 2!");
      expect(alice.RatchetDecrypt(headerB1, messageB1, ad)).toEqual("Hi Alice!");
    });
    ...
    test("Skip a too many messages", () => {
      ...
      // then
      expect(() => bob.RatchetDecrypt(header, message, ad)).toThrow("Too many skipped messages!")
    });
  });
});
